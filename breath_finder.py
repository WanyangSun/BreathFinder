...
in_path, out_path = be.gen_workspace(filename)

# In[2] 读取mzXML质谱数据
ms, ms2 = load_xml.load_mzxml_file(in_path)
# 拆分ms正负离子数据，获取ms极性信息
ms1, mspola = es.sep_polarity(ms)

# In[3] 测试某一离子获取呼气信号和背景信号的准确度
# target_mass输入格式：0 = 总离子流，xxx.xxxx = 目标离子

# 方法一：同时提取两个离子模式的数据：
id_p, id_n = be.check_all_sig(ms1, mspola, signal_weight=0.8)
...
df_p, df_n, mzlist_unique = be.extract_all_mz(out_path, ms1, mspola, id_p, id_n, intensity_thre=5e4, min_num=5)

# 不生成时间-强度矩阵&信号提取矩阵
df_p, df_n, mzlist_unique = be.extract_all_mz(out_path, ms1, '+', id_p, id_n, intensity_thre=1e5, ext_matrix=False, ext_signal=False)
